...
/**
 * 主题 zip 上传安装：multipart 字段 file。校验通过后解压进 themes/<slug>，
 * 不合规由服务层抛 ValidationError 且自动删除临时文件，不污染程序目录。
 */
export async function POST(req: Request) {
  ...
  try {
    ...
    const theme = await themes.installThemeZip(buf);
    return ok({ data: theme }, 201);
  } catch (e) {
